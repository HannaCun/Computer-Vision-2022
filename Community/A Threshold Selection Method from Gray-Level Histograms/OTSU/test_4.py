import time

import numpy as np
import cv2 as cv
import matplotlib.pyplot as plt
from PIL import Image
from itertools import permutations,combinations
# 将图片转为灰度图
img = cv.imread('2.png', 0)
cv.imshow("img", img)
# cv.waitKey()
# plt.figure()
# plt.hist(img.ravel(),256)
# plt.title('original_hist')



def OTSU(img_gray, GrayScale):
    assert img_gray.ndim == 2, "must input a gary_img"  # shape有几个数字, ndim就是多少
    img_gray = np.array(img_gray).ravel().astype(np.uint8)
    u1 = 0.0  # 背景像素的平均灰度值
    u2 = 0.0  # 前景像素的平均灰度值
    u3 = 0.0
    u4 = 0.0
    th1 = 0.0
    th2 = 0.0
    th3 = 0.0

    # 总的像素数目
    PixSum = img_gray.size
    # 各个灰度值的像素数目
    PixCount = np.zeros(GrayScale)
    # 各灰度值所占总像素数的比例
    PixRate = np.zeros(GrayScale)
    # 统计各个灰度值的像素个数
    for i in range(PixSum):
        # 默认灰度图像的像素值范围为GrayScale
        Pixvalue = img_gray[i]
        PixCount[Pixvalue] = PixCount[Pixvalue] + 1

    # 确定各个灰度值对应的像素点的个数在所有的像素点中的比例。
    for j in range(GrayScale):
        PixRate[j] = PixCount[j] * 1.0 / PixSum
    Max_var = 0
    nums = [i for i in range(GrayScale)]
    #print(nums)
    res = comb(nums)
    print("len(res)",len(res))
    lst = []
    lstth1=[]
    lstth2=[]
    for i in range(len(res)):
        #print(i)
        u1_tem = 0.0
        u2_tem = 0.0
        u3_tem = 0.0
        u4_tem = 0.0
        tmp1 = res[i][0]
        tmp2 = res[i][1]
        tmp3 = res[i][2]
        #print(tmp1,tmp2)
        w1 = np.sum(PixRate[:tmp1])
        w2 = np.sum(PixRate[tmp1:tmp2])
        w3 = np.sum(PixRate[tmp2:tmp3])
        w4 = 1 - w1 - w2 - w3
        #print(type(w3),w3)
        if w1 == 0 or w2 == 0 or w3 == 0 or w4 ==0:
            pass
        else:
            # 类别1的平均像素
            for m1 in range(tmp1):
                u1_tem = u1_tem + PixRate[m1] * m1
            u1 = u1_tem * 1.0/w1
            #print(type(u1), u1)
            for m2 in range(tmp1,tmp2):
                u2_tem = u2_tem + PixRate[m2] * m2
            u2 = u2_tem *1.0 /w2

            for m3 in range(tmp2,tmp3):
                u3_tem = u3_tem + PixRate[m3] * m3
            u3 = u3_tem * 1.0 /w3

            for m4 in range(tmp3,GrayScale):
                u4_tem = u4_tem + PixRate[m4] * m4
            u4 = u4_tem * 1.0 /w4
            #print(type(u3), u3)
            ut = w1*u1 + w2*u2 + w3*u3 + w4*u4
            #print(type(ut),ut)
            tem_var = w1*(u1-ut)*(u1-ut)+w2*(u2-ut)*(u2-ut)+w3*(u3-ut)*(u3-ut)+w4*(u4-ut)*(u4-ut)
            if Max_var<tem_var:
                Max_var = tem_var
                th1 = tmp1
                th2 = tmp2
                th3 = tmp3
    # 确定最大类间方差对应的阈值
    return th1,th2,th3

def comb(nums):
    res = []
    for i in combinations(nums,3):
        res.append(list(i))
    return res
# nums = [i for i in range(4)]
# print(comb(nums))

t = time.time()
th1,th2,th3 = OTSU(img,256)
t1 = time.time() - t
print(t1)
print(th1,th2,th3)
im = np.array(img)
for i in range(im.shape[0]):
    for j in range(im.shape[1]):
        if im[i][j] < th1:
            im[i][j] = 0
        elif im[i][j]>=th1 and im[i][j]<th2:
            im[i][j] = 0.5 *(th1 + th2)
        elif im[i][j]>=th2 and im[i][j]<th3:
            im[i][j] = 0.5*(th2 + th3)
        else:
            im[i][j] = 255

plt.imshow(Image.fromarray(im) , cmap='gray')
plt.title('OTSU_FourthThresholds')
plt.figure()
plt.hist(im.ravel(),256)
plt.title('hist')

plt.show()