...
def OTSU(img_gray, GrayScale):
    ...
    for i in range(len(res)):
        ...
        w3 = np.sum(PixRate[tmp2:tmp3])
        w4 = np.sum(PixRate[tmp3:tmp4])
        w5 = 1 - w1 - w2 - w3-w4
        #print(type(w3),w3)
        if w1 == 0 or w2 == 0 or w3 == 0 or w4 ==0 or w5 ==0:
            ...
            for m3 in range(tmp2,tmp3):
                u3_tem = u3_tem + PixRate[m3] * m3
            u3 = u3_tem * 1.0 /w3

            for m4 in range(tmp3,tmp4):
                u4_tem = u4_tem + PixRate[m4] * m4
            u4 = u4_tem * 1.0 /w4

            for m5 in range(tmp4,GrayScale):
                u5_tem = u5_tem + PixRate[m5] * m5
            u5 = u5_tem * 1.0 /w5
            #print(type(u3), u3)
            ut = w1*u1 + w2*u2 + w3*u3 + w4*u4 + w5*u5
            #print(type(ut),ut)
            tem_var = w1*(u1-ut)*(u1-ut)+w2*(u2-ut)*(u2-ut)+w3*(u3-ut)*(u3-ut)+w4*(u4-ut)*(u4-ut)+w5*(u5-ut)*(u5-ut)
            if Max_var<tem_var:
                Max_var = tem_var
                th1 = tmp1
                th2 = tmp2
...
for i in range(im.shape[0]):
    for j in range(im.shape[1]):
        if im[i][j] < th1:
            ...
            im[i][j] = 0.5 *(th1 + th2)
        elif im[i][j]>=th2 and im[i][j]<th3:
            im[i][j] = 0.5*(th2 + th3)
        elif im[i][j]>=th3 and im[i][j]<th4:
            im[i][j] = 0.5*(th3+th4)
        else:
            im[i][j] = 255

plt.imshow(Image.fromarray(im) , cmap='gray')
plt.title('OTSU_FifthThresholds')
plt.figure()
